...
module.exports = function (_a, options) {
    var _b;
    ...
    const code2 = new CodeBuilder(opts.indent);
    if (!opts.noTags) {
    ...
    switch (postData.mimeType) {
        case 'application/x-www-form-urlencoded':
            code2.push("'form_params' => ".concat((0, helpers_1.convertType)(postData.paramsObj, opts.indent + opts.indent, opts.indent), ","));
            break;
        case 'multipart/form-data': {
            ...
            if (fields_1.length) {
                code2.push("'multipart' => ".concat((0, helpers_1.convertType)(fields_1, opts.indent + opts.indent, opts.indent)));
        ...
        default:
            if (postData.text) {
                code2.push("'body' => ".concat((0, helpers_1.convertType)(postData.text), ","));
            }
    }
    // construct headers
    ...
    if (headers.length) {
        code2.push("'headers' => [");
        code2.push(headers.join('\n'));
        code2.push('],');
    }
    code.push('$client = new \\GuzzleHttp\\Client();');
    code.blank();
    if (code.code.length) {
        code.push("$response = $client->request('".concat(method, "', '").concat(fullUrl, "', ["));
        code.push(code2.join());
        code.push(']);');
    }
    else {
        code.push("$response = $client->request('".concat(method, "', '").concat(fullUrl, "');"));
